...
export class LocalDataService extends DataService {
  ...
  async updateUserSettings(userId: number, settings: Record<string, string | number | boolean>): Promise<void> {
  ...
  async getById<T>(collection: string, id: string | number): Promise<T | null> {
    const items = await this.query<T>(collection, {});
    return items.find((item: T & { id: string | number }) => item.id === id) || null;
  }
  ...
  async query<T>(collection: string, filters: Record<string, any>): Promise<T[]> {
    ...
    if (Object.keys(filters).length > 0) {
      items = items.filter((item: T & Record<string, unknown>) => {
  ...
  async update<T>(collection: string, id: string | number, data: Partial<T>): Promise<T> {
    const items = await this.query<T>(collection, {});
    const index = items.findIndex((item: T & { id: string | number }) => item.id === id);
  ...
  async delete(collection: string, id: string | number): Promise<void> {
    ...
    const filteredItems = items.filter((item: T & { id: string | number }) => item.id !== id);
  ...
  private getCourseVersions(courseId: number): Array<{ version: number; courseData: Course; timestamp: number }> {
