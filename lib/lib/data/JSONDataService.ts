...
export class JSONDataService extends DataService {
  ...
  async get(collection: string, id: string | number): Promise<unknown> {
  ...
  async query<T>(storeName: string, filters?: Record<string, unknown>): Promise<T[]> {
    this.ensureInitialized();
  ...
  private matchesFilters(item: Record<string, unknown>, filters: Record<string, unknown>): boolean {
  ...
  private getCourseVersions(courseId: number): Array<{ version: number; courseData: Course; createdAt: number }> {
