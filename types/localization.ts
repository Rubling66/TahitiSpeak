...
export interface ContentLocalization {
  ...
  languageCode: string;
  localizedFields: Record<string, string | number | boolean | Date>;
...
export interface LocalizationAPI {
  ...
  getTranslationQualityReport(languageCode: string, dateRange?: { start: Date; end: Date }): Promise<{ score: number; issues: string[]; suggestions: string[] }>;
  getTranslationCostAnalysis(dateRange?: { start: Date; end: Date }): Promise<{ totalCost: number; breakdown: Record<string, number>; trends: Record<string, number[]> }>;
