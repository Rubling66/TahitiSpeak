...
export function useI18n() {
  ...
  const translateRich = useCallback((key: string, values?: Record<string, unknown>) => {
    try {
  ...
  const applyRTLStyles = useCallback((styles: Record<string, unknown>) => rtlUtils.applyStyles(styles), [rtlUtils]);
