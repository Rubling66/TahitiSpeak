...
export interface UseTranslationManagerReturn {
  ...
  // Advanced features
  formatMessage: (key: string, values?: Record<string, string | number | boolean>, options?: { fallback?: string; plural?: number }) => string;
  hasTranslation: (key: string, namespace?: string) => boolean;
  getTranslationMetadata: (key: string) => { key: string; locale: string; lastModified: Date | null; source: string };
}
...
export function useTranslationManager(
  ...
  const dynamicTranslations = useRef<Record<string, Record<string, string>>>({});
   ...
   */
  const getTranslationMetadata = useCallback((key: string): { key: string; locale: string; lastModified: Date | null; source: string } => {
