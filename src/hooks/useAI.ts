...
import { TranslationRequest, PronunciationFeedback, LessonContent } from '../lib/genkit/AIService';
  ...
  const analyzePronunciation = useCallback(async (targetText: string, userAudio: string, language: 'fr' | 'ty'): Promise<PronunciationFeedback> => {
    ...
    try {
      const response = await fetch('/api/ai/pronunciation', {
        ...
        body: JSON.stringify({ targetText, userAudio, language }),
      });
  ...
  const generateLesson = useCallback(async (
    ...
    try {
      const response = await fetch('/api/ai/lesson', {
        ...
        body: JSON.stringify({ topic, level, focusAreas }),
      });
  ...
  const getConversationSuggestions = useCallback(async (
    ...
    try {
      const response = await fetch('/api/ai/suggestions', {
        ...
        body: JSON.stringify({ context, userLevel }),
      });
