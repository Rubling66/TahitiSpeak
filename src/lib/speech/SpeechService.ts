...
// Enhanced with Local AI TTS capabilities

import { localTTSService, LocalTTSOptions } from '../local-ai/LocalTTSService';
...
export class SpeechService {
  ...
  async speak(
    ...
    // Try to use local AI TTS first, fallback to browser TTS
    try {
      if (localTTSService.isAvailable()) {
        const localOptions: LocalTTSOptions = {
          language: this.mapLanguageToLocal(options.lang),
          speed: options.rate,
          pitch: options.pitch,
          volume: options.volume
        };
        
        const response = await localTTSService.generateSpeech({
          text,
      ...
      console.warn('Local AI TTS failed, falling back to browser TTS:', error);
    }
  ...
  cleanup(): void {
    this.stopListening();
    this.stopSpeaking();
    // Clear local TTS cache
    localTTSService.clearCache();
  }

  /**
   * Map browser language codes to local AI language options
   */
  private mapLanguageToLocal(lang?: string): 'french' | 'tahitian' | 'english' {
    if (!lang) return 'french';
    
    const langCode = lang.toLowerCase();
    if (langCode.startsWith('fr')) return 'french';
    if (langCode.startsWith('ty')) return 'tahitian';
   ...
   */
  async getPhoneticGuide(text: string, language?: string): Promise<string> {
    try {
      const lang = this.mapLanguageToLocal(language);
      return await localTTSService.getPhoneticGuide(text, lang);
    } catch (error) {
